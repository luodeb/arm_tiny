...
{
    tiny_printf(INFO, "[VIRTIO_IRQ] Initializing VirtIO interrupt system\n");
    ...
    tiny_printf(INFO, "[VIRTIO_TEST] GIC enable status for IRQ %d: %s\n",
                VIRTIO_IRQ_0, gic_enabled ? "ENABLED" : "DISABLED");

    if (!gic_enabled)
    {
        tiny_printf(ERROR, "[VIRTIO_TEST] Failed to enable IRQ %d in GIC\n", VIRTIO_IRQ_0);
        return false;
    }
    enable_interrupts();
    tiny_printf(INFO, "[VIRTIO_IRQ] Global interrupts enabled\n");

    // Mark interrupts as enabled
    virtio_irq_state.interrupts_enabled = true;

    tiny_printf(INFO, "[VIRTIO_IRQ] VirtIO interrupt system initialization COMPLETED\n");
...
void virtio_irq_handler(uint64_t *ctx)
{
    ...
    tiny_printf(INFO, "[VIRTIO_IRQ] *** VirtIO INTERRUPT #%d RECEIVED ***\n",
                virtio_irq_state.interrupt_count);
    ...
    if (!dev)
    {
        tiny_printf(ERROR, "[VIRTIO_IRQ] ERROR: No VirtIO device available in interrupt handler\n");
        return;
    }

    tiny_printf(INFO, "[VIRTIO_IRQ] Processing interrupt for device at 0x%x\n", dev->base_addr);
    ...
    tiny_printf(INFO, "[VIRTIO_IRQ] Interrupt status register: 0x%x\n", interrupt_status);

    // Decode interrupt status bits with detailed logging
    if (interrupt_status & VIRTIO_IRQ_VRING_UPDATE)
    {
        tiny_printf(INFO, "[VIRTIO_IRQ] VRING_UPDATE interrupt: Used buffer notification\n");
    }
    if (interrupt_status & VIRTIO_IRQ_CONFIG_CHANGE)
    {
        tiny_printf(INFO, "[VIRTIO_IRQ] CONFIG_CHANGE interrupt: Device configuration changed\n");
    }
    if (interrupt_status == 0)
    {
        tiny_printf(WARN, "[VIRTIO_IRQ] WARNING: Spurious interrupt (status=0)\n");
    }

    // Acknowledge interrupt to VirtIO device
    if (interrupt_status != 0)
    {
        tiny_printf(DEBUG, "[VIRTIO_IRQ] Acknowledging interrupt status 0x%x\n", interrupt_status);
        ...
        tiny_printf(DEBUG, "[VIRTIO_IRQ] Status after ACK: 0x%x (should be 0)\n", status_after_ack);
    }

    // Set interrupt received flag (this wakes up waiting functions)
    virtio_irq_state.interrupt_received = true;

    tiny_printf(INFO, "[VIRTIO_IRQ] Interrupt processing COMPLETED successfully\n");
}
...
bool virtio_wait_for_interrupt(uint32_t timeout_ms)
{
    ...
    tiny_printf(DEBUG, "[VIRTIO_WAIT] Timeout loops: %d, debug interval: %d\n",
    ...
    while (!virtio_irq_state.interrupt_received && loop_count < timeout_loops)
    {
        ...
        if (debug_interval > 0 && (loop_count % debug_interval) == 0)
        {
            ...
            tiny_printf(DEBUG, "[VIRTIO_WAIT] Progress: %d%% (%d/%d loops)\n",
    ...
    if (virtio_irq_state.interrupt_received)
    {
        ...
        tiny_printf(INFO, "[VIRTIO_WAIT] SUCCESS: Interrupt received after %d loops (%d.%d ms)\n",
                    loop_count, ms_elapsed, us_remainder);
        tiny_printf(INFO, "[VIRTIO_WAIT] Interrupt status: 0x%x, count: %d\n",
                    virtio_irq_state.interrupt_status, virtio_irq_state.interrupt_count);
        return true;
    }
    else
    {
        tiny_printf(WARN, "[VIRTIO_WAIT] TIMEOUT: No interrupt received within %d ms (%d loops)\n",
...
uint32_t virtio_get_interrupt_status(void)
{
    ...
    if (!dev)
    {
        tiny_printf(ERROR, "[VIRTIO_IRQ] ERROR: No VirtIO device for status read\n");
        return 0;
    }

    uint32_t status = virtio_read32(dev->base_addr + VIRTIO_MMIO_INTERRUPT_STATUS);
    tiny_printf(DEBUG, "[VIRTIO_IRQ] Current interrupt status: 0x%x\n", status);
    return status;
}

void virtio_reset_interrupt_state(void)
{
    tiny_printf(DEBUG, "[VIRTIO_IRQ] Resetting interrupt state\n");
    ...
    tiny_printf(DEBUG, "[VIRTIO_IRQ] Interrupt state reset completed\n");
}

void virtio_print_interrupt_stats(void)
{
    tiny_printf(INFO, "[VIRTIO_IRQ] === VirtIO Interrupt Statistics ===\n");
    tiny_printf(INFO, "[VIRTIO_IRQ] Target IRQ number: %d (calculated for slot 31)\n", VIRTIO_IRQ_0);
    tiny_printf(INFO, "[VIRTIO_IRQ] Interrupts enabled: %s\n",
                virtio_irq_state.interrupts_enabled ? "YES" : "NO");
    tiny_printf(INFO, "[VIRTIO_IRQ] Total interrupts received: %d\n",
                virtio_irq_state.interrupt_count);
    tiny_printf(INFO, "[VIRTIO_IRQ] Last interrupt status: 0x%x\n",
                virtio_irq_state.interrupt_status);
    tiny_printf(INFO, "[VIRTIO_IRQ] Interrupt received flag: %s\n",
                virtio_irq_state.interrupt_received ? "TRUE" : "FALSE");
    tiny_printf(INFO, "[VIRTIO_IRQ] Last used index: %d\n",
    ...
    tiny_printf(INFO, "[VIRTIO_IRQ] GIC interrupt %d enabled: %s\n",
                target_irq, gic_enabled ? "YES" : "NO");

    // Check current device interrupt status
    uint32_t current_status = virtio_get_interrupt_status();
    tiny_printf(INFO, "[VIRTIO_IRQ] Current interrupt status: 0x%x\n", current_status);

    // Additional system diagnostics
    tiny_printf(INFO, "[VIRTIO_IRQ] === System State Diagnostics ===\n");
    ...
    if (dev)
    {
        ...
        tiny_printf(INFO, "[VIRTIO_IRQ] VirtIO device status: 0x%x\n", device_status);

        // Check if device is in DRIVER_OK state
        if (device_status & VIRTIO_STATUS_DRIVER_OK)
        {
            tiny_printf(INFO, "[VIRTIO_IRQ] Device is DRIVER_OK\n");
        }
        else
        {
            tiny_printf(WARN, "[VIRTIO_IRQ] Device is NOT DRIVER_OK (current: 0x%x)\n", device_status);
        }
        ...
        {
            tiny_printf(INFO, "[VIRTIO_IRQ] Queue avail flags: 0x%x (0=interrupts enabled)\n",
                        queue->avail->flags);
        }
    }
    ...
    {
        tiny_printf(INFO, "[VIRTIO_IRQ] Handler registered for IRQ %d: 0x%x\n",
                    VIRTIO_IRQ_0, (uint64_t)g_handler_vec[VIRTIO_IRQ_0]);
    }
    else
    {
        tiny_printf(ERROR, "[VIRTIO_IRQ] NO HANDLER registered for IRQ %d!\n", VIRTIO_IRQ_0);
    }

    tiny_printf(INFO, "[VIRTIO_IRQ] =====================================\n");
}
