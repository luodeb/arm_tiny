...
bool virtio_test_basic_access(void)
{
    ...
    tiny_log(DEBUG, "[VIRTIO] Magic value: 0x%x\n", magic);

    uint32_t version = virtio_read32(dev->base_addr + VIRTIO_MMIO_VERSION);
    tiny_log(DEBUG, "[VIRTIO] Version: 0x%x\n", version);
    ...
    if (queue != NULL && queue->avail != NULL) {
        tiny_log(DEBUG, "[VIRTIO] Queue avail addr: 0x%p\n", queue->avail);
        tiny_log(DEBUG, "[VIRTIO] Avail idx: %d\n", queue->avail->idx);
    }
    
    if (queue != NULL && queue->used != NULL) {
        tiny_log(DEBUG, "[VIRTIO] Queue used addr: 0x%p\n", queue->used);
        tiny_log(DEBUG, "[VIRTIO] Used idx: %d\n", queue->used->idx);
    }

    tiny_log(DEBUG, "[VIRTIO] Basic access test completed\n");
    return true;
}
...
{
    tiny_log(DEBUG, "[VIRTIO] Testing potential hang points...\n");

    // Test 1: Simple counter loop
    tiny_log(DEBUG, "[VIRTIO] Test 1: Simple counter loop\n");
    for (int i = 0; i < 1000; i++) {
        if (i % 100 == 0) {
            tiny_log(DEBUG, "[VIRTIO] Counter: %d\n", i);
        }
    }
    
    // Test 2: Memory barrier operations
    tiny_log(DEBUG, "[VIRTIO] Test 2: Memory barriers\n");
    ...
    if (dev != NULL && dev->ready) {
        for (int i = 0; i < 100; i++) {
            ...
            if (i % 20 == 0) {
                tiny_log(DEBUG, "[VIRTIO] Status: 0x%x\n", status);
            }
        }
    } else {
        tiny_log(DEBUG, "[VIRTIO] Device not ready, skipping register access\n");
    }
    ...
    if (queue != NULL && queue->used != NULL) {
        for (int i = 0; i < 10; i++) {
            ...
            tiny_log(DEBUG, "[VIRTIO] Used idx read %d: %d\n", i, used_idx);
        }
    } else {
        tiny_log(DEBUG, "[VIRTIO] Queue not ready, skipping memory access\n");
    }

    tiny_log(DEBUG, "[VIRTIO] Hang point tests completed\n");
    return true;
}
